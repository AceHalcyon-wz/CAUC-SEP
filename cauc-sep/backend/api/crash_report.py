...
def _get_storage() -> CrashReportStorage:
    ...
    if not storage:
        raise HTTPException(
            status_code=503,
            detail="Crash report storage not initialized"
        )
    return storage
...
async def list_crash_reports(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list crash reports: {str(e)}"
        )
...
async def get_crash_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get crash statistics: {str(e)}"
        )
...
async def get_crash_report(report_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get crash report: {str(e)}"
        )
...
async def update_crash_report_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update crash report status: {str(e)}"
        )
...
async def export_crash_report(report_id: str):
    ...
    try:
        ...
        if not filepath:
            raise HTTPException(
                status_code=404,
                detail=f"Crash report not found or export failed: {report_id}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to export crash report: {str(e)}"
        )
...
async def cleanup_old_crash_reports(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to cleanup crash reports: {str(e)}"
        )
