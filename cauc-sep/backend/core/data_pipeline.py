...
class RingBuffer:
            ...
            if write_len == 0:
                ...
                if self._overflow_strategy == OverflowStrategy.DROP_NEW:
                    ...
                    if data_len >= self._size:
                        ...
                        self._buffer[self._write_index : self._write_index + write_len] = data[:write_len]
                        self._count = self._size
                        return write_len
    ...
    def read_zero_copy(self, count: int) -> tuple[NDArray[np.floating[Any] | np.integer[Any]] | None, int]:
        """
        零拷贝读取指定数量的数据。
    ...
    def get_contiguous_view(self) -> tuple[NDArray[np.floating[Any] | np.integer[Any]] | None, int, int]:
        """
        获取连续数据视图（零拷贝）。
...
class StreamProcessor:
    ...
    def process(self, data: NDArray[np.floating[Any] | np.integer[Any]]) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._cached_data is None or not self._cache_valid or self._last_buffer_count != current_buffer_count:
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            "avg_trigger_check_time_ms": float(np.mean(list(self._trigger_check_times))) if self._trigger_check_times else 0.0,
            "avg_process_time_ms": float(np.mean(list(self._process_times))) if self._process_times else 0.0,
            "trigger_check_p95_ms": get_percentile(self._trigger_check_times, 95),
            "process_p95_ms": get_percentile(self._process_times, 95),
        }

    def process_batch(self, data_list: list[NDArray[np.floating[Any] | np.integer[Any]]]) -> dict[str, Any]:
        """
        批量处理多个数据数组。
...
class DataPipeline:
    ...
    def add_trigger(
        ...
        Args:
            ...
            cache_result: 是否缓存条件结果
        """
        self._processor.add_trigger(name, trigger_type, condition, callback, check_interval, cache_result)
    ...
    def get_channel_data(self, channel: str = "default") -> NDArray[np.floating[Any] | np.integer[Any]]:
        """
        获取指定通道的数据。
