...
def test_overflow_strategies():
    ...
    print(f"OVERWRITE_OLDEST: 写入{written}点，缓冲区使用率: {buffer1.available / buffer1.capacity * 100:.1f}%")
    ...
    print(f"DROP_NEW: 写入{written}点，缓冲区使用率: {buffer2.available / buffer2.capacity * 100:.1f}%")
