...
文件名: crash_report.py
路径: backend/api/
功能: 崩溃报告管理API，提供崩溃报告的查询、统计、更新和导出接口
作者: CAUC-SEP Team
创建日期: 2024-01-01
依赖: FastAPI, pydantic, core.crash_report

主要功能：
- 崩溃报告列表查询（多条件过滤、分页）
- 崩溃报告详情查看（完整堆栈、系统环境）
- 崩溃报告状态更新（新报告/已确认/已解决/已忽略）
- 崩溃报告统计分析（按严重程度、状态、异常类型分布）
- 崩溃报告导出（压缩JSON格式）
- 过期报告清理（自动/手动清理）

API端点：
- GET /: 查询崩溃报告列表
- GET /statistics: 获取崩溃报告统计信息
- GET /{report_id}: 获取崩溃报告详情
- PUT /{report_id}/status: 更新崩溃报告状态
- GET /{report_id}/export: 导出崩溃报告到文件
- POST /cleanup: 清理过期崩溃报告
- GET /severity/levels: 获取严重程度级别列表
- GET /status/values: 获取状态值列表
