...
文件名: example_api.py
路径: backend/api/
功能: API规范优化示例，展示如何使用统一响应格式、参数校验和异常处理
版本: v1.0
...
示例内容：
1. 基础查询接口（GET方法）- 展示统一响应格式
2. 操作接口（POST方法）- 展示参数校验和异常处理
3. 分页查询接口 - 展示分页响应格式
4. 错误响应示例 - 展示不同类型的错误响应
...
async def list_devices(
    ...
    status: str | None = Query(None, description="设备状态过滤"),
) -> ApiResponse[dict]:
    """
    设备列表查询示例。
    ...
    2. 使用Query参数进行过滤和分页
    3. 返回统一格式的分页响应
    4. 响应格式: {success: true, data: {items: [], total: int, ...}, error: null}
