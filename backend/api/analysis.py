...
文件名: analysis.py
路径: backend/api/
功能: 数据分析API，提供信号处理、曲线拟合、磁滞回线分析、报告生成等接口
作者: CAUC-SEP Team
创建日期: 2024-01-01
依赖: FastAPI, numpy, core.analysis, api.schemas, middleware.security

主要功能：
- 信号平滑（Savitzky-Golay滤波、巴特沃斯低通滤波）
- 曲线拟合（Langevin函数、线性拟合）
- 磁滞回线分析（Hc矫顽力、Mr剩磁、Ms饱和磁矩）
- 多模型拟合对比（双曲正切、反正切、Braunbeck、Langevin模型）
- 分析报告生成与导出（JSON、CSV、PDF格式）
- 历史数据查询（多条件筛选、统计分析）
- 数据集对比（差异指标计算、相关性分析）
...
- POST /smooth: 信号平滑处理
- POST /fit: 曲线拟合
- POST /hysteresis: 磁滞回线分析
- POST /multi-fit: 多模型拟合对比
- POST /report/generate: 生成分析报告
- POST /report/export: 导出分析报告
- GET /history: 查询历史数据
- POST /compare: 对比多个数据集

安全加固：
- SubTask 13.1: 输入验证增强（数据数组长度限制MAX_DATA_POINTS=100000）
- 数据长度一致性校验
- 数据点最小数量验证
- 模型类型白名单验证
"""
