...
文件名: piezo.py
路径: backend/api/
功能: 压电陶瓷控制API，提供电压/位移控制、校准管理等接口
作者: CAUC-SEP Team
创建日期: 2024-01-01
依赖: FastAPI, core.piezo_controller, core.device_registry, api.schemas
...
API端点：
- GET /status: 获取压电陶瓷控制器状态
- POST /voltage: 设置电压值（开环模式）
- GET /voltage: 获取当前电压值
- POST /displacement: 设置位移值（闭环模式）
- GET /displacement: 获取当前位移值
- POST /control_mode: 切换控制模式
- POST /calibration/perform: 执行校准
- GET /calibration: 获取校准数据
- DELETE /calibration: 清除校准数据
- POST /calibration/point: 添加校准点
- GET /calibration/points: 获取校准点列表
