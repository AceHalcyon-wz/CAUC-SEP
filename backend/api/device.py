...
文件名: device.py
路径: backend/api/
功能: 设备管理API，提供设备列表、状态查询、IO端口配置等接口
作者: CAUC-SEP Team
创建日期: 2024-01-01
依赖: FastAPI, pydantic, core.abstract, core.dm2c_driver, core.electromagnet_driver, core.picoammeter, core.piezo_controller, core.temperature_controller, middleware.security
...
支持的设备类型：
- stepper_motor: 雷赛DM2C步进电机
- electromagnet: 电磁铁控制器
- temperature_controller: 温控系统
- piezo_controller: 压电陶瓷控制器
- picoammeter: 微电流计

API端点：
- GET /list: 获取设备列表
- GET /{device_id}/status: 获取指定设备状态
- GET /{device_id}/io/di/functions: 获取DI功能代码列表
- GET /{device_id}/io/do/functions: 获取DO功能代码列表
- POST /{device_id}/io/di/configure: 配置DI端口功能
- POST /{device_id}/io/do/configure: 配置DO端口功能
- GET /{device_id}/io/di/status: 读取所有DI端口状态
- GET /{device_id}/io/do/status: 读取所有DO端口状态
- GET /{device_id}/io/di/{di_number}/config: 读取指定DI端口配置
- GET /{device_id}/io/do/{do_number}/config: 读取指定DO端口配置

安全加固：
- SubTask 13.1: 输入验证增强（device_id格式验证，仅允许字母、数字、下划线、连字符）
- 设备存在性验证
- 设备初始化状态检查
"""
