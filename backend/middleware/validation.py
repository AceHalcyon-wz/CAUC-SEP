...
创建日期：2026-03-08
更新日期：2026-03-14
依赖：re, html, logging, pydantic (可选)

使用示例：
    >>> from middleware.validation import sanitize_input, detect_sql_injection
    >>> result = sanitize_input("<script>alert('xss')</script>")
    >>> print(result.sanitized_value)  # 输出清理后的安全文本
    >>> is_injection, patterns = detect_sql_injection("SELECT * FROM users")
"""
...
SQL_INJECTION_PATTERNS = [
    ...
    # SQL关键字组合
    re.compile(
        r"\b(union\s+select|select\s+.*\s+from|insert\s+into|delete\s+from|drop\s+table|truncate\s+table)\b",
        re.IGNORECASE,
    ),
    # SQL函数
