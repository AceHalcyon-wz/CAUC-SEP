...
使用示例：
    >>> from middleware.cors_config import setup_cors, CORSEnvironment
    >>> app = FastAPI()
    >>> setup_cors(app, environment=CORSEnvironment.PRODUCTION)
"""
...
def get_cors_config(environment: CORSEnvironment | None = None) -> CORSConfig:
    ...
    if environment is None:
        ...
        environment = (
            CORSEnvironment(env_str)
            if env_str in [e.value for e in CORSEnvironment]
            else CORSEnvironment.DEVELOPMENT
        )
...
class SecureCORSMiddleware(BaseHTTPMiddleware):
    ...
    def _cleanup_cache(self) -> None:
        ...
        expired_keys = [
            key
            for key, (time, _) in self._preflight_cache.items()
    ...
    def _log_request(self, request: Request, origin: str) -> None:
        ...
        """
        logger.debug(
            f"CORS request: method={request.method}, " f"path={request.url.path}, origin={origin}"
        )
...
def validate_cors_security(config: CORSConfig) -> list[str]:
    ...
    if "*" in config.allow_origins:
        if config.allow_credentials:
            ...
            warnings.append("警告: allow_origins=['*'] 允许任意源访问，仅适用于公开API")
    ...
    for origin in config.allow_origins:
        if any(pattern in origin for pattern in dev_patterns):
            warnings.append(f"警告: 源 '{origin}' 包含开发环境地址，不应在生产环境使用")

    # 检查是否使用HTTP（非HTTPS）
    for origin in config.allow_origins:
        if origin.startswith("http://") and "localhost" not in origin and "127.0.0.1" not in origin:
            warnings.append(f"警告: 源 '{origin}' 使用HTTP而非HTTPS，存在中间人攻击风险")

    # 检查是否暴露敏感头
    sensitive_headers = {"authorization", "cookie", "set-cookie"}
    for header in config.expose_headers:
        if header.lower() in sensitive_headers:
            warnings.append(f"警告: 暴露敏感头 '{header}' 可能导致信息泄露")
