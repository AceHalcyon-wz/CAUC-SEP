...
文件名: main.py
路径: backend/
功能: FastAPI主程序入口，API路由与WebSocket服务
版本: v0.3.0
...
async def lifespan(app: FastAPI):
    ...
    # 初始化数据存储
    storage = DataStorage(str(get_db_path("experiments.db")))
    ...
    # 初始化链路追踪系统
    init_tracing(db_path=str(get_db_path("traces.db")))
    ...
    init_crash_report_manager(
        ...
        app_version="0.3.0",
        db_path=str(get_db_path("crash_reports.db")),
        auto_cleanup=True,
        cleanup_days=30,
        install_hook=True,
    )
