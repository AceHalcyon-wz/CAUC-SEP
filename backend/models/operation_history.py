...
class OperationHistory(Base):
    ...
    def get_detail(self) -> dict:
        ...
        将JSON格式的操作详情字符串解析为字典对象。
        如果解析失败或详情为空，返回空字典。

        Returns:
            dict: 操作详情字典，包含操作的具体参数和状态信息

        Example:
            >>> history = OperationHistory(operation_detail='{"key": "value"}')
            >>> detail = history.get_detail()
            >>> print(detail)  # {'key': 'value'}
        """
        import json
    ...
    def set_detail(self, detail: dict) -> None:
        ...
        将字典对象序列化为JSON字符串存储到operation_detail字段。
        使用ensure_ascii=False支持中文内容。

        Args:
            detail: 操作详情字典，包含操作的具体参数和状态信息

        Example:
            >>> history = OperationHistory()
            >>> history.set_detail({"action": "move", "position": 100})
            >>> print(history.operation_detail)  # '{"action": "move", "position": 100}'
        """
        import json
