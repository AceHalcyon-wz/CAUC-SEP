...
目前的系统架构已实现了硬件控制与数据采集，但在数据采集后的"原位分析"（In-situ Analysis）能力上尚属空白。对于自旋电子学实验，通常需要实时计算磁矩、拟合磁滞回线、提取矫顽力（H_c）和剩磁（M_r）等参数。

**改进目标**：

1. **原位分析**：在数据采集过程中实时进行数学变换（如微分、积分、FFT）
2. **物理模型拟合**：集成标准物理模型（如Langevin函数、Braunbeck函数）用于磁化曲线拟合
3. **报告自动化**：自动生成包含关键参数的实验摘要图（Publication-ready Figures）

### 13.2 技术选型与库引入

为了保持打包体积（<200MB）和授权合规性，本方案仅引入开源Python库，完全替代MATLAB方案。

**功能需求与推荐库对照表**：

| 功能需求       | 推荐库     | 版本建议 | 作用说明                             | 部署影响   |
| -------------- | ---------- | -------- | ------------------------------------ | ---------- |
| 核心数值计算   | NumPy      | v1.26+   | (已有)高性能矩阵运算，数据预处理基础 | 无(已包含) |
| 数据分析处理   | Pandas     | v2.2+    | (已有)数据清洗、结构化存储、CSV导出  | 无(已包含) |
| 曲线拟合与优化 | LMFIT      | v1.2+    | 新增：提供高级非线性最小二乘法拟合   | +2MB       |
| 科学绘图       | Matplotlib | v3.8+    | 新增：生成高质量2D图表               | +15MB      |
| 信号处理       | SciPy      | v1.10+   | 建议升级：提供FFT、滤波器设计        | +5MB       |

> **注意**：引入上述库后，通过 Nuitka 打包的总增量预计在 30MB 以内，仍满足 <200MB 的设计目标。

### 13.3 模块架构设计

建议在后端 `core/` 目录下新增 `analysis.py` 模块，构建"分析引擎"。
...
class PhysicsAnalyzer:
    ...
    def smooth_signal(self, y_data, method='savgol', window_length=11, polyorder=2):
        """
        信号平滑处理
        method: 'savgol'(Savitzky-Golay滤波), 'butter'(巴特沃斯低通)
        """
        if method == 'savgol':
            return signal.savgol_filter(y_data, window_length, polyorder)
        elif method == 'butter':
            b, a = signal.butter(3, 0.1, 'low')
            return signal.filtfilt(b, a, y_data)

    def fit_hysteresis(self, x_field, y_moment):
        """
        磁滞回线拟合 (简化模型示例)
        实际应用中可使用更复杂的物理模型
        """
        # 1. 寻找饱和区进行线性拟合(背景扣除)
        # 2. 计算关键参数: Hc (矫顽力), Mr (剩磁), Ms (饱和磁矩)
        hc = self._calculate_coercivity(x_field, y_moment)
        mr = self._calculate_remanence(y_moment)
        return {"Hc": hc, "Mr": mr, "Fit_Curve": fitted_y}
```

#### 13.3.2 关键算法实现：磁滞回线分析

针对自旋电子器件，重点实现以下算法：

1. **背景扣除 (Background Subtraction)**：线性或多项式拟合高场区数据，扣除线性背景（源于顺磁/抗磁杂质）
2. **参数提取**：

   - **M_r (剩磁)**：磁场 H=0 时的磁矩平均值
   - **H_c (矫顽力)**：磁矩 M=0 时对应的磁场值（通过插值法精确计算）
   - **M_sat (饱和磁矩)**：高场区磁矩的平均值

### 13.4 前后端交互流程优化

为了让前端（Vue3）展示分析结果，需要扩展 API 接口。

#### 13.4.1 新增 REST API 接口

在 `api/experiment.py` 中增加：

- **POST /api/v1/analysis/smooth**

  - 功能：对指定实验数据进行平滑处理
  - 请求体：包含 exp_id, method, params 等参数
  - 响应：返回处理后的数据点数组
- **POST /api/v1/analysis/fit**

  - 功能：执行物理模型拟合
  - 响应：包含 Hc, Mr, R_squared 等参数及图片URL

#### 13.4.2 前端（Vue3）功能增强

在前端界面增加 "数据分析" 选项卡：

1. **参数配置面板**：允许用户选择滤波窗口大小、拟合模型类型
2. **实时预览**：调用 API 后，使用 ECharts 动态对比"原始数据"与"处理后数据"
3. **报告导出**：增加"导出分析报告"按钮，后端生成 Matplotlib 图片并返回下载链接

### 13.5 优化方案总结

通过引入上述 Python 科学计算栈，你的平台将实现从"单纯的数据记录仪"到"智能物理实验工作站"的跨越。

**优势**：

- **零成本**：所有库均为开源免费，无需购买 MATLAB 许可证
- **一体化**：所有分析在 FastAPI 后端完成，用户无需安装额外软件
- **扩展性强**：未来可轻松加入机器学习（PyTorch/TensorFlow Lite）进行异常检测或自动参数优化

**建议实施步骤**：

1. 在 requirements.txt 中添加 lmfit 和 matplotlib
2. 修改 build.bat，确保 Nuitka 打包时包含这些库（通常自动检测）
3. 编写 core/analysis.py 核心算法
4. 扩展 FastAPI 路由
5. 前端增加分析页面
...
- **实验元数据**：保留SQLite，但增加JSON-LD语义化标记（描述样品ID、磁场方向、温度曲线等），便于后续AI分析
...
- 基于Optuna或Ray Tune实现实验参数自动优化（如自动寻找最佳PID温控参数）
- 异常检测：使用Isolation Forest实时检测异常磁矩跳变（可能源于接触不良）

#### 14.6.2 自然语言交互

- 集成本地LLM（如Llama 3.2 3B，通过 `llama-cpp-python`），支持语音/文字指令："将磁场设定为1.5T并开始测量"，由LLM解析为API调用序列。
...
| 长期 | P3     | AI辅助参数优化 + 自动报告生成      | 实验效率提升30%，减少人工值守            |

### 14.8 API架构与插件架构的关系辨析
